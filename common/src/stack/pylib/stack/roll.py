...
class KickstartFile:
    ...
    def generate(self, start):
        ...
        if self.root:
            os.environ["Kickstart_LocalRolldir"] = os.path.join("/", self.root)
        os.environ["Node_Hostname"] = "BOOTSTRAP"  # silences errors

        cwd = os.getcwd()
        os.chdir(os.path.join(self.dist.name, self.dist.arch, "build"))
        list = []
        cmd = "kpp %s %s | kgen %s" % (self.kppFlags, start, self.kgenFlags)
...
class Distribution:
    ...
    def generate(self, md5=True, resolve=True):
        ...
        if resolve:
            flags += " resolve=true"
        stack.util.system(
            "/opt/stack/bin/stack create distribution %s %s" % (self.name, flags)
        )
        self.tree = stack.file.Tree(os.path.join(os.getcwd(), self.getPath()))
