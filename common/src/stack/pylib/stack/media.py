...
class Media:
        ...
        for r, d, f in os.walk("/mnt/cdrom"):
            for fname in f:
                if regexp.match(fname):
                    xmlfile = stack.file.RollInfoFile("%s/%s" % (r, fname))
                    ...
                    roll_list.append((rollname, rollversion, rollrelease, rollarch))
    ...
    def listRolls(self, url, diskid, rollList):
        ...
        cmd = "%s --timeout=15 --tries=2 -O - -nv %s 2>&1 " % (wget, url)
        ...
        for line in os.popen(cmd).readlines():
            ...
            if len(filename) > 0 and filename[-1] == "/":
                ...
                if len(urlList) >= 4 and urlList[-1] == "redhat":
                    ...
                    self.listRolls(os.path.join(url, dir), diskid, rollList)
