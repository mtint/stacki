...
class Boxes(BaseModel):
    id = AutoField(column_name='ID')
    name = CharField(column_name='Name', constraints=[SQL("DEFAULT 'default'")], index=True)
...
class Nodes(BaseModel):
    appliance = ForeignKeyField(column_name='Appliance', field='id', model=Appliances, null=True)
...
class ScopeMap(BaseModel):
    appliance = ForeignKeyField(column_name='appliance_id', field='id', model=Appliances, null=True)
...
class FirewallRules(BaseModel):
    ...
    in_subnet = ForeignKeyField(column_name='in_subnet_id', field='id', model=Subnets, null=True)
    name = CharField(index=True)
    out_subnet = ForeignKeyField(backref='subnets_out_subnet_set', column_name='out_subnet_id', field='id', model=Subnets, null=True)
    protocol = CharField()
...
class FirmwareMake(BaseModel):
    name = CharField(index=True)
    version_regex = ForeignKeyField(column_name='version_regex_id', field='id', model=FirmwareVersionRegex, null=True)
...
class Networks(BaseModel):
    ...
    subnet = IntegerField(column_name='Subnet', null=True)
...
class IbMemberships(BaseModel):
    ...
    class Meta:
        ...
        indexes = (
            (('switch', 'part_name', 'interface'), False),
        )
...
class Partitions(BaseModel):
    ...
    partition_flags = CharField(column_name='PartitionFlags', constraints=[SQL("DEFAULT ''")])
    partition_id = CharField(column_name='PartitionID', constraints=[SQL("DEFAULT ''")])
    partition_size = CharField(column_name='PartitionSize', constraints=[SQL("DEFAULT ''")])
...
class Routes(BaseModel):
    ...
    netmask = CharField()
    scope_map = ForeignKeyField(column_name='scope_map_id', field='id', model=ScopeMap)
    subnet = ForeignKeyField(column_name='subnet_id', field='id', model=Subnets, null=True)
