    ...
    def validate(self, hosts, interface=None, mac=None, network=None):
        ...
        for host in hosts:
            ...
            if mac and mac.lower() not in host_data[host]["macs"]:
                raise CommandError(
                    self, f'mac "{mac}" does not exist for host "{host}"'
                )

            if network and network.lower() not in host_data[host]["networks"]:
                raise CommandError(
                    self, f'network "{network}" does not exist for host "{host}"'
                )
            ...
            if len(combo) > 1 and combo not in host_data[host]["combos"]:
                ...
                raise CommandError(
                    self,
                    f'combination of "{combo_join}" does not exist for host "{host}"',
                )
