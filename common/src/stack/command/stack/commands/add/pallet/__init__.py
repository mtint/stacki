...
class Command(command):
    ...
    def copy(self, clean, prefix, updatedb, URL):
        ...
        p = subprocess.run(
            ["find", "%s" % self.mountPoint, "-type", "f", "-name", "roll-*.xml"],
        ...
        for key, info in dict.items():
            self.runImplementation(
                "native_%s" % info.getRollOS(), (clean, prefix, info)
            )
    ...
    def clean_ludicrous_packages(self):
        ...
        p = subprocess.Popen(
            _command.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
    ...
    def run(self, params, args):
        ...
        if (
            not isolist
            and not network_pallets
            and not disk_pallets
            and not network_isolist
        ):
            #
        ...
        for iso in network_isolist:
            ...
            cwd = os.getcwd()
            os.system(
                "mount -o loop %s %s > /dev/null 2>&1" % (local_path, self.mountPoint)
            )
        ...
        if isolist:
            ...
            for iso in isolist:  # have a set of iso files
                cwd = os.getcwd()
                os.system(
                    "mount -o loop %s %s > /dev/null 2>&1" % (iso, self.mountPoint)
                )
                self.copy(clean, dir, updatedb, iso)
                os.chdir(cwd)
                os.system("umount %s > /dev/null 2>&1" % self.mountPoint)
        ...
        self.endOutput(
            header=["name", "version", "release", "arch", "os"], trimOwner=False
        )
