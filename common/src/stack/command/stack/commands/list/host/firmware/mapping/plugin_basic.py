    ...
    def run(self, args):
        ...
        if make:
            ...
            if not self.owner.make_exists(make=make):
                raise ParamError(
                    cmd=self.owner, param="make", msg=f"The make {make} doesn't exist."
                )
        ...
        if versions:
            ...
            try:
                self.owner.ensure_firmwares_exist(
                    make=make, model=model, versions=versions
                )
            except CommandError as exception:
                raise ArgError(cmd=self.owner, arg="version", msg=exception.message())
